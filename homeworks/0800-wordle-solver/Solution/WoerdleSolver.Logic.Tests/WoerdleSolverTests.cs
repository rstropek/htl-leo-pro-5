...
public class WoerdleSolverTests
{
    ...
    {
        var solver = new Solver();
        Assert.Throws<ArgumentException>("word", () => solver.StoreResult("YXYXY", "?????"));
    }
    ...
    public void StoreResultNothingCorrect()
    {
        ...
        Assert.True(Enumerable.Range(0, 5).All(ix => "APFEL".All(c => !solver.PossibleValues[ix].Contains(c))));
    }
    ...
    public void StoreResultEverythingCorrect()
    {
        ...
        Assert.True("APFEL".Select((c, ix) => (c, ix)).All(r => 
            solver.PossibleValues[r.ix].Count == 1 && solver.PossibleValues[r.ix].First() == r.c));
    }
    ...
    public void PossibleWordsNoSolution()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => solver.PossibleWords());
    }
}
